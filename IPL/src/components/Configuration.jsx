...
const Configuration = ({ players, poolSize, setPoolSize, configTime, setConfigTime }) => {
  ...
  const handlePoolSizeChange = (event) => {
    ...
    if (newSize >= 1 && newSize <= players.length) {
      setPoolSize(newSize); // Update only if within valid range
  ...
  return (
    <div className="configure-player-list">
      ...
      <div className="pool-size-container">
        ...
        <input
          type="number"
          value={poolSize} // Use value instead of defaultValue to reflect state changes
